...
function VNodeCache(node, ref, cref) {
...
function vNodeCreate(node, context) {
  ...
  if ((flags & V_TEXT) !== 0) {
    ref = document.createTextNode(node.data);
  } else if ((flags & V_ELEMENT) !== 0) {
    if ((flags & V_SVG) === 0) {
      ref = document.createElement(node.tag);
    } else {
      ref = document.createElementNS('http://www.w3.org/2000/svg', node.tag);
    }
  } else if ((flags & V_COMPONENT) !== 0) {
    cref = new node.tag(context, node.data, node.children);
    cref.create();
    cref.init();
    ref = cref.element;
  }

  return new VNodeCache(node, ref, cref);
}

/**
 * Render internal representation of the VNode.
 *
 * @param {VNodeCache} node
 * @param {Component} context
 */
  ...
  if ((flags & (V_ELEMENT | V_COMPONENT | V_ROOT)) !== 0) {
    ...
    if ((flags & V_COMPONENT) !== 0) {
      ...
      for (i = 0, il = node.children.length; i < il; i++) {
        var c = vNodeCreate(node.children[i], context);
        node.childrenCache.push(c);
        vNodeInsertChild(node, c, null, context);
      }
    }
  }
}
...
function vNodeUpdate(a, b, context) {
  ...
  if ((flags & V_TEXT) !== 0) {
    ...
    if ((flags & V_COMPONENT) !== 0) {
      ...
      updateChildren(a, a.childrenCache, b.children, context);
    }
  }
}
 ...
 * Insert VNode.
 *
 * @param {VNodeCache} parent Parent node.
 * @param {VNodeCache} node Node to insert.
 ...
 *
 * @param {VNodeCache} parent Parent node.
 * @param {VNodeCache} node Node to move.
 * @param {Node} nextRef Reference to the next html element.
 */
 ...
 *
 * @param {VNodeCache} parent Parent node.
 * @param {VNodeCache} node Node to remove.
 */
...
function vNodeDispose(node) {
  if ((node.flags & V_COMPONENT) !== 0) {
    node.cref.dispose();
  } else if (node.childrenCache != null) {
    for (var i = 0; i < node.childrenCache.length; i++) {
      vNodeDispose(node.childrenCache[i]);
    }
  }
}
...
function vNodeAttach(node) {
  vNodeAttached(node);
  if (((node.flags & V_COMPONENT) === 0) && (node.childrenCache != null)) {
    for (var i = 0; i < node.childrenCache.length; i++) {
      vNodeAttach(node.childrenCache[i]);
    }
  }
}
 ...
 * in runtime error.
 *
 * @param {VNodeCache} parent Parent.
 * @param {Array.<VNodeCache>} a Old children list.
...
function updateChildren(parent, a, b, context) {
  ...
  if (a != null && a.length !== 0) {
    if (b == null || b.length === 0) {
      ...
      if (a.length === 1 && b.length === 1) {
        ...
        if ((aNode.key == null && vNodeSameType(aNode, bNode)) ||
          ...
          bNodeCache = vNodeCreate(bNode, context);
          vNodeInsertChild(parent, bNodeCache, null, context);
          a[0] = bNodeCache;
        }
      } else if (a.length === 1) {
        ...
        if (aNode.key == null) {
          while (i < b.length) {
            ...
            if (vNodeSameType(aNode, bNode)) {
              vNodeUpdate(aNode, bNode, context);
              newChildrenCache.push(aNode);
              updated = true;
              break;
            }
            bNodeCache = vNodeCreate(bNode, context);
            newChildrenCache.push(bNodeCache);
            vNodeInsertChild(parent, bNodeCache, aNode.ref, context);
          }
        } else {
          while (i < b.length) {
            bNode = b[i++];
        ...
        if (updated) {
          while (i < b.length) {
            bNodeCache = vNodeCreate(b[i++], context);
            newChildrenCache.push(bNodeCache);
            vNodeInsertChild(parent, bNodeCache, null, context);
          }
        } else {
    ...
    parent.childrenCache = [];
    for (i = 0; i < b.length; i++) {
      bNodeCache = vNodeCreate(b[i], context);
      parent.childrenCache.push(bNodeCache);
      vNodeInsertChild(parent, bNodeCache, null, context);
    }
  }
}
...
function updateImplicitChildren(parent, a, b, context) {
  ...
  var nextPos;
  var next;
  var newChildrenCache = new Array(b.length);
  var bNodeCache;
  ...
  while (aStart <= aEnd && bStart <= bEnd) {
    ...
    if (vNodeSameType(aNode, bNode)) {
      ...
      bNodeCache = vNodeCreate(bNode, context);
      newChildrenCache[bStart] = bNodeCache;
      vNodeInsertChild(parent, bNodeCache, aNode.ref, context);
      vNodeRemoveChild(parent, aNode);
    }
    aStart++;
  ...
  while (bStart <= bEnd) {
    bNodeCache = vNodeCreate(b[bStart], context);
    newChildrenCache[bStart] = bNodeCache;
    vNodeInsertChild(parent, bNodeCache, next, context);
    bStart++;
  }
...
function updateExplicitChildren(parent, a, b, context) {
  ...
  outer: do {
    ...
    while (aStartNode.key === bEndNode.key) {
      ...
      nextPos = bEnd + 1;
      next = nextPos < b.length ? newChildrenCache[nextPos].ref : null;
      vNodeMoveChild(parent, aStartNode, next);
      aStart++;
      bEnd--;
      if (aStart > aEnd || bStart > bEnd) {
        break outer;
    ...
    while (aEndNode.key === bStartNode.key) {
      ...
      vNodeMoveChild(parent, aEndNode, aStartNode.ref);
      aEnd--;
      bStart++;
      if (aStart > aEnd || bStart > bEnd) {
        break outer;
    ...
    if (moved) {
      ...
      for (i = bLength - 1; i >= 0; i--) {
        if (sources[i] === -1) {
          ...
          nextPos = pos + 1;
          next = nextPos < b.length ? newChildrenCache[nextPos].ref : null;
          bNodeCache = vNodeCreate(node, context);
          newChildrenCache[pos] = bNodeCache;
          vNodeInsertChild(parent, bNodeCache, next, context);
        } else {
          if (j < 0 || i != seq[j]) {
            pos = i + bStart;
            bNodeCache = newChildrenCache[pos];
            nextPos = pos + 1;
            next = nextPos < b.length ? newChildrenCache[nextPos].ref : null;
            vNodeMoveChild(parent, bNodeCache, next);
          } else {
            j--;
          }
        }
...
Component.prototype.updateRoot = function(newRoot) {
  if (this.root == null) {
    this.root = new VNodeCache(newRoot, this.element, this);
    if ((this.flags & C_MOUNTING) !== 0) {
      vNodeMount(this.element, this);
      this.flags &= ~C_MOUNTING;
    } else {
      vNodeRender(this.root, this);
    }
  } else {
